  ...
  return (
    <main>
      ...
      <section className="hero">
        ...
        <img src="/mi-scout-gold-logo.png" alt="MI Scout watermark" className="watermark" />
        <div className="container">
          <nav className="nav">
            <div className="brand">
              <img src="/mi-scout-gold-logo.png" alt="MI Scout" />
              ...
              <Card className="report-card">
                <div className="report-inner">
                  <div className="report-head">
                    <div><p>Monthly Intelligence</p><h2>Elite Club Report</h2></div>
                    <Crown className="gold-icon" />
                  </div>
                  <div className="stats">
                    {[
                      ['100', 'Hidden Players'], ['20', 'Full Reports'], ['8', 'Markets'], ['4', 'Priority Tiers']
                    ].map(([n,l]) => <div className="stat" key={l}><strong>{n}</strong><span>{l}</span></div>)}
                  </div>
                  <p className="green-box">Built for sporting directors, recruitment departments and ambitious clubs looking for early-market advantage.</p>
                </div>
              </Card>
            </motion.div>
          </div>
      ...
      <section className="container pain">
        {painPoints.map(p => <div className="pain-card" key={p}><AlertTriangle /><p>{p}</p></div>)}
      </section>
        ...
        <div className="container">
          ...
          <div className="grid3">
            {services.map(item => {
              ...
              return <Card key={item.title}><Icon className="green-icon" /><h3>{item.title}</h3><p>{item.text}</p></Card>
            })}
          </div>
        </div>
      </section>
        ...
        <div className="two-col">
          ...
          <Card>
            ...
            <div className="pdf-preview">
              <div className="pdf-top"><div><p>CONFIDENTIAL PROFILE</p><h4>Hidden Winger Profile</h4><span>RW / LW · 19 · Europe</span></div><div className="score">MI Score<br/><b>8.4</b></div></div>
              {['Role Fit: High','Market Risk: Medium','Transfer Priority: Tier 1','Upside: High resale potential'].map(x => <div className="metric" key={x}>{x}</div>)}
            </div>
          </Card>
        </div>

        <div className="player-section">
          <div className="section-head small"><span>Player Cards</span><h3>Recruitment shortlist carousel</h3></div>
          <div className="grid3">
            {playerCards.map(player => <Card key={player.name}>
              <div className="player-head"><div><span>{player.tag}</span><h4>{player.name}</h4></div><div className="mini-score"><Star size={16} />{player.score}</div></div>
              <div className="player-info"><p><Activity size={16}/>{player.role}</p><p><Calendar size={16}/>Age {player.age}</p><p><MapPin size={16}/>{player.market}</p></div>
      ...
      <section className="section alt">
        <div className="container two-col">
          <div><span className="eyebrow">Method</span><h2>From scouting need to ranked shortlist.</h2></div>
          <div className="steps">{process.map((step, i) => <div className="step" key={step}><b>{i+1}</b><p>{step}</p></div>)}</div>
        </div>
      </section>
      ...
      <section id="authority" className="section container">
        <div className="two-col">
          <div>
            ...
            <p className="muted">MI Scout is not content, media or fan-based analysis. It is a focused recruitment intelligence service built for real club decision making.</p>
            ...
            ].map(x => <div key={x}><CheckCircle2 /> {x}</div>)}</div>
          </div>
      ...
      <section id="case-study" className="section container">
        <div className="section-head">
          <span>Case Study</span><h2>From longlists to signable targets</h2>
          <p>Example of how MI Scout converts broad scouting needs into clear, actionable recruitment decisions.</p>
        </div>
        <div className="case-grid">
          <div className="before"><p>Before MI Scout</p><h3>Noise, time loss, unclear priorities</h3><ul><li>150+ player names from multiple sources</li><li>No clear tactical fit filtering</li><li>Time wasted on video screening</li><li>No transfer timing insight</li></ul></div>
          <div className="after"><p>After MI Scout</p><h3>5-player actionable shortlist</h3><ul><li>100-player filtered database</li><li>20 detailed scout reports</li><li>Top 5 priority targets identified</li><li>Clear sign / monitor / drop decisions</li></ul></div>
        </div>
        <div className="grid3">
      ...
      <section id="packages" className="section container">
        <div className="section-head center"><span>Pricing</span><h2>Simple packages for serious recruitment.</h2></div>
        <div className="grid3">
          {packages.map(pkg => <Card key={pkg.name} className={pkg.featured ? 'featured' : ''}>
            <div className="pkg-head"><h3>{pkg.name}</h3>{pkg.featured ? <Euro /> : <Lock />}</div>
            <h4>{pkg.price}</h4><p>{pkg.period}</p><span>{pkg.note}</span>
            <div className="pkg-features">{pkg.features.map(f => <div key={f}><CheckCircle2 />{f}</div>)}</div>
          </Card>)}
        </div>
      </section>
      ...
      <section id="contact" className="contact">
        <div className="logo-center"><img src="/mi-scout-gold-logo.png" alt="MI Scout" /></div>
        <div className="container two-col">
          <div>
          ...
          <Card>
            ...
            <form action="https://formspree.io/f/xojrqqad" method="POST" className="form">
              ...
              <select name="interest"><option>Interested in...</option><option>Starter Intelligence</option><option>Elite Monthly Intelligence</option><option>Custom Recruitment Mission</option></select>
              <textarea name="message" placeholder="Tell us your recruitment need"></textarea>
              <button type="submit" className="btn btn-solid">Send Request <ArrowRight size={18}/></button>
            </form>
          </Card>
        </div>
      </section>
